    ...
    <div class="py-8 bg-gradient-to-br from-unib-blue-50 to-gray-50 min-h-screen">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-unib-blue-100">
                ...
                {{-- Card Body --}}
                <div class="p-6">
                    <form method="POST" action="{{ route('superadmin.periodes.update', $periode) }}" class="space-y-6" id="periodeForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" name="tahun_akademik" value="{{ old('tahun_akademik', $periode->tahun_akademik) }}" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="semester_ke" id="semester_ke" value="{{ old('semester_ke', $periode->semester_ke) }}" 
                            ...
                            <div>
                                ...
                                <select name="semester_type" id="semester_type" required 
                                        class="w-full border-gray-300 rounded-lg shadow-sm focus:border-unib-blue-500 focus:ring-unib-blue-500 px-4 py-3 text-base transition duration-200">
                                    <option value="ganjil" @selected(old('semester_type', $periode->semester_type) === 'ganjil')>Ganjil</option>
                                    <option value="genap" @selected(old('semester_type', $periode->semester_type) === 'genap')>Genap</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" name="tanggal_mulai_kp" value="{{ old('tanggal_mulai_kp', $periode->tanggal_mulai_kp->format('Y-m-d')) }}" required
                                       class="w-full border-gray-300 rounded-lg shadow-sm focus:border-unib-blue-500 focus:ring-unib-blue-500 px-4 py-3 text-base transition duration-200">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="tanggal_selesai_kp" value="{{ old('tanggal_selesai_kp', $periode->tanggal_selesai_kp->format('Y-m-d')) }}" required
                                       class="w-full border-gray-300 rounded-lg shadow-sm focus:border-unib-blue-500 focus:ring-unib-blue-500 px-4 py-3 text-base transition duration-200">
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            semesterTypeSelect.addEventListener('change', function() {
                ...
                const expectedType = semesterKe % 2 === 1 ? 'ganjil' : 'genap';
                
                if (selectedType !== expectedType) {
                    semesterInfo.innerHTML += ' ⚠️ (Tidak sesuai pola normal)';
                    semesterInfo.className = 'text-sm text-yellow-600 mt-2 flex items-center font-medium';
                } else {
                    updateSemesterType(); // Reset to normal info
                }
            });
        });
    </script>
